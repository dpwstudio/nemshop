...
{/if}
	<div class="shopping_cart">
		<a class="{if $cart_qties == 0}cart-empty{/if}" href="{$link->getPageLink($order_process, true)|escape:'html':'UTF-8'}" title="{l s='View my shopping cart' mod='blockcart'}" rel="nofollow">
					...
					<div class="cart_block_list{if isset($blockcart_top) && !$blockcart_top}{if isset($colapseExpandStatus) && $colapseExpandStatus eq 'expanded' || !$ajax_allowed || !isset($colapseExpandStatus)} expanded{else} collapsed unvisible{/if}{/if}">
									...
									<dt data-id="cart_block_product_{$product.id_product|intval}_{if $product.id_product_attribute}{$product.id_product_attribute|intval}{else}0{/if}_{if $product.id_address_delivery}{$product.id_address_delivery|intval}{else}0{/if}" class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">
										...
										<div class="cart-info">
											<div class="product-name">
												<span class="quantity-formated"><a class="cart_block_product_name" href="{$link->getProductLink($product, $product.link_rewrite, $product.category, null, null, $product.id_shop, $product.id_product_attribute)|escape:'html':'UTF-8'}" title="{$product.name|escape:'html':'UTF-8'}">{$product.name|truncate:30:'...'|escape:'html':'UTF-8'}</a>
											</div>
											...
											{/if}
											<span class="quantity">{$product.cart_quantity}</span>&nbsp;x&nbsp;</span>
											<span class="price">
						...
						<div class="cart-prices">
							<div class="cart-prices-line first-line">
							    <span class="title-cart-prices{if !($page_name == 'order-opc') && $shipping_cost_float == 0 && (!$cart_qties || $cart->isVirtualCart() || !isset($cart->id_address_delivery) || !$cart->id_address_delivery || $free_ship)} unvisible{/if}">
							...
							{if $show_tax && isset($tax_cost)}
								<div class="cart-prices-line">
								    <span class="title-cart-prices">{l s='Tax' mod='blockcart'}</span>
									<span class="price cart_block_tax_cost ajax_cart_tax_cost">{$tax_cost}</span>
									
								</div>
							{/if}
							<div class="cart-prices-line last-line">
							    <span class="title-cart-prices title-cart-prices-total">{l s='Total' mod='blockcart'}</span>
	...
	<div id="layer_cart">
		<div class="clearfix">
			<div class="layer_cart_product col-xs-12 col-md-6">
				...
				<span class="title">
					<i class="icon-check-circle"></i>{l s='Product successfully added to your shopping cart' mod='blockcart'}
				</span>
