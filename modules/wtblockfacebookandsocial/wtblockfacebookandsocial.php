...
class WTBlockFacebookAndSocial extends Module
{
public function __construct()
{
		$this->name = 'wtblockfacebookandsocial';
		$this->tab = 'others';
		$this->version = '1.0.0';
		...
		$this->bootstrap = true;
		parent::__construct();
		$this->displayName = $this->l('WT Facebook block And Social Footer');
		$this->description = $this->l('Displays a block for subscribing to your Facebook page and Social on footer.');
...
public function uninstall()
{
		...
		Configuration::deleteByName('facebook_url');
...
{
		if (!$this->isCached('wtblockfacebookandsocial.tpl', $this->getCacheId()))
		{
			$facebookurl = Configuration::get('facebook_url');
			if (!strstr($facebookurl, 'facebook.com'))
				$facebookurl = 'https://www.facebook.com/'.$facebookurl;
		...
		return $this->display(__FILE__, 'views/templates/hook/wtblockfacebookandsocial.tpl', $this->getCacheId());
}

public function hookHeader()
{
			$this->page_name = Dispatcher::getInstance()->getController();
			$this->context->controller->addCss($this->_path.'views/css/wtblockfacebookandsocial.css');
