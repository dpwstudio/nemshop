...
class WtCustomHtml extends Module
{
	...
	private $_html;
	private $myHook = array('displayNav','displayTop','displayTopColumn','displayHookSpecial','displayBanner','displayLeftColumn','displayRightColumn','displayFooter','displayHome', 'displayTopHome', 'displayBottomHome', 'displayBottomFooter', 'displayCopyRight', 'displayTopFooter', 'displayTopMenu');
	...
	public function hookdisplayHookSpecial()
	{
		if (!$this->isCached('wtcustomhtml_hookspecial.tpl', $this->getCacheId('wtcustomhtml_hookspecial')))
		{
			$block_list = $this->getBlockFromHook('displayHookSpecial');
			
			$this->context->smarty->assign(array(
				'block_list' => $block_list
			));
		}
		return $this->display(__FILE__, 'wtcustomhtml_hookspecial.tpl', $this->getCacheId('wtcustomhtml_hookspecial'));
	}
	...
	{
		if (!$this->isCached('wtcustomhtml_displaytopmenu.tpl', $this->getCacheId('wtcustomhtml_displaytopmenu')))
		{
			$block_list = $this->getBlockFromHook('displayTopMenu');
			$this->context->smarty->assign(array(
				'block_list' => $block_list
			));
		}
		return $this->display(__FILE__, 'wtcustomhtml_displaytopmenu.tpl', $this->getCacheId('wtcustomhtml_displaytopmenu'));
	}
